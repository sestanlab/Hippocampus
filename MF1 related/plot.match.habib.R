...
seu <- readRDS(file = paste0(inputdir, "Habib_Human_DFC_HIP_seu.rds")) %>%
			subset(region == 'HIP') %>%
			subset(cluster != "remove")
seu  <- seu[, !seu@meta.data$cluster %in% c("ExN_PFC1","ExN_PFC2")]


Idents(seu) <- "cluster"
pub_avg <- log(AverageExpression(seu)$RNA + 1)
pub_exp <- rownames(seu)[Matrix::rowSums(seu$RNA@counts!=0) >= 15]


pub_order <- c("GC", "ExN CA3", "ExN CA1", "InN1", "InN2", "Astro1", "Astro2", "NSC","OPC", "Oligo1", "Oligo2", "Micro", "Endo")


hvg <- rownames(hipec$pca@feature.loadings)
gene_use <- intersect(hvg, pub_exp)
pub_avg <- pub_avg[gene_use, pub_order]
colnames(pub_avg) <- gsub("^GC", "DG GC", colnames(pub_avg)) %>%
						gsub("NSC", "Ependymal", .)


cor_mat <- cor(avgs[gene_use, ], pub_avg, method = "p")

pdf(paste0(outputdir, "SF1.Match.habib.heatmap.pdf"), width = 4, height = 7)
