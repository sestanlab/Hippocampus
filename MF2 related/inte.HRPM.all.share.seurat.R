...
file_name <- paste0("HRPM.all.seurat.expHVG.1500")
...
if (!file.exists(inte.hrm.file)){
	...
	data_list <- lapply(allsps, function(sp) {
	...
	## Do the integration
	seu <- Integratelist.seurat(obj.list = gclist, hvg = hrpm_hvg, file_name = file_name, input_dir = inputdir, inte.dims = 1:25, cluster.dims = 1:25, reference = which(names(gclist) %in% c("MMBold", "HSB237")))
}
