...
if (!file.exists(auc_file)){
	...
	all_auc <- lapply(names(seulist), function(sp) {
		auc <- GetModuleScore(assay.data = seulist[[sp]]$RNA@data, features = mars, seed = 42, method = "aucell", input_dir = inputdir, file_name = paste0("DCX.AUC.full.", sp), output_dir = outputdir)$auc[colnames(seulist[[sp]]), ]
...
p <- ggplot(new_auc, aes(x = newcls, fill = DCX, y = AUC)) +
		...
		stat_compare_means(label = "p.signif", method = "wilcox.test", size = 1.5, method.args = list(alternative = "less"), symnum.args = list(cutpoints = c(0, 0.01, 1), symbols = c("**", "ns"))) +
...
pdf(paste0(outputdir, "SF2.DCX.pos.neg.comparison.full.pdf"), width = 4, height = 1.2)
print(p)
dev.off() 
